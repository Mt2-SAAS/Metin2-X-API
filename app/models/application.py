"""Modelos para la gestión de sitios web de juegos en línea."""
...
class Download(BaseSaveModel):
    """Modelo para gestionar las descargas del juego."""
    ...
    id = Column(
        ...
        primary_key=True,
        autoincrement=True,
        comment='ID único de la descarga'
    )
    provider = Column(
        String(100),
        nullable=False,
        comment='Proveedor de la descarga (ej: Google Drive, Mega, etc.)'
    )
    ...
    published = Column(
        ...
        default=False,
        nullable=False,
        comment='Indica si la descarga ha sido publicada'
    )
    category = Column(
        String(50),
        nullable=False,
        comment='Categoría de la descarga (ej: cliente, parches, herramientas)'
    )
...
class Image(BaseSaveModel):
    """Modelo para gestionar las imágenes del sitio web."""
...
class Pages(BaseSaveModel):
    """Modelo para gestionar las páginas estáticas del sitio web."""
    __tablename__ = 'pages'
...
class Site(BaseSaveModel):
    """Modelo para gestionar la configuración del sitio web."""
    __tablename__ = 'sites'
